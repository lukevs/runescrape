...
TABLE_URLS_BY_ITEM_TYPE = {
    ...
    RunscrapeItemType.HANDS: "https://oldschool.runescape.wiki/w/Worn_Equipment#Hands_slot ",
}
...
class RunscrapeSpider(scrapy.Spider):
    ...
    def parse_article(self, response):
        ...
        full_image_source_path = response.css(".mw-parser-output .floatleft img")[0].attrib["src"]
        full_image_source_url = f"https://{WIKI_DOMAIN}{full_image_source_path}"

        yield RunescrapeItem(
            title=title,
            item_type=item_type,
            icon_source_url=icon_source_url,
            full_image_source_url=full_image_source_url
        )
